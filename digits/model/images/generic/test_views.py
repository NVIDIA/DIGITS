...
class TestCreate(BaseTestCase):
    ...
    def setupClass(cls):
        ...
        with app.test_request_context():
            cls.url = flask.url_for('generic_image_model_create')

        dj = mock.Mock(spec=digits.dataset.GenericImageDatasetJob)
        dj.status.is_running.return_value = True
        dj.id.return_value = 'dataset'
        dj.name.return_value = ''

        mj = mock.Mock(spec=digits.model.GenericImageModelJob)
