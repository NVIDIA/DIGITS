...
deleting completed volatile jobs
"""
VOLATILE_JOB_DELETE_TIMEOUT_SECONDS = 3600
        ...
        try:
            ...
            while not self.shutdown.is_set():
                ...
                if not last_saved or time.time()-last_saved > 15:
                    for job in self.jobs.values():
                        if job.status.is_running():
                            if not job.is_volatile():
                                job.save()
                        elif job.is_volatile() and time.time() - job.status_history[-1][1] > VOLATILE_JOB_DELETE_TIMEOUT_SECONDS:
